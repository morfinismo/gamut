...
var timePicker = function timePicker(dateBox, config) {
  ...
  $tp.meridiamValue = hourValue > 23 ? "am" : hourValue > 11 ? "pm" : "am"; //set value to the $tp.input
  ...
  $tp.open = function (top, left, height, width) {
    //rotate handler
    var rotateHand = function rotateHand(event) {
      var elem = event.target;
      elem.style.cursor = "grabbing";
      console.dir(elem);
      var rotating = true;
      var clock = document.querySelector(".gamut__timePicker__clock");
      var rect = clock.getBoundingClientRect(); // get clock size and position

      var radius = rect.width / 2; // calculate radius based on size

      var rotateHandler = function rotateHandler(e) {
        var radians = Math.atan2(e.pageX - (rect.x + radius), e.pageY - (rect.y + radius));
        var rotateDegrees = radians * (180 / Math.PI) * -1 - 180;

        if (rotating) {
          elem.style.transform = "rotate(".concat(rotateDegrees, "deg)");
        }
      };

      document.addEventListener("mousemove", rotateHandler);

      var cancelRotateHandler = function cancelRotateHandler(event) {
    ...
    var getTimeParamValue = function getTimeParamValue(timeParam) {
      if (timeParam === "hour") {
    ...
    var toggleMeridiam = function toggleMeridiam(e) {
      var newValue = e.target.dataset.meridiamValue;
      var meridiams = document.querySelectorAll(".gamut__timePicker__meridiamSelector--meridiam");
      meridiams.forEach(function (meridiam) {
    ...
    widget.classList.add("gamut__timePicker"); //create the meridiam selector
    ...
    var minRotateDegree = 360 / minutePointers;
    var clockSize = 202;
    var rotateValue = 90;
    ...
    for (var _i = 0; _i < totalLabels; _i++) {
      var label = newElem("span");
      label.innerHTML = hourValue;

      var _angle = _i * (Math.PI / (totalLabels / 2));
    ...
    widget.appendChild(timeDisplay); //done button
